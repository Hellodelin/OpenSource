 ...
 * instance associated with the current web application.
 * 在一个特定请求初次处理的时候，通过FacesContextFactory的getFacesContext方法的调用与其建立关联
 * The instance remains active until its <code>release()</code> method is called, after
 * which no further references to this instance are allowed.  
 * 这个实例一直存活到调用其release之前，之后不允许对这个实例的任何引用
 * While a {@link FacesContext} instance is active, it must not be referenced
...
public abstract class FacesContext {
	 ...
	 * 这个Map与request没有关联，换句话说，这里对属性的修改不会体现到request中
     * 
    ...
    public Map<Object, Object> getAttributes() {
        ...
        if (!isCreatedFromValidFactory) {//非合法工厂创建
